...
<script type="text/ecmascript-6">
  ...
  export default {
    mixins: [playlistMixin],  // 混合 (mixins) 是一种分发 Vue 组件中可复用功能的非常灵活的方式。混合对象可以包含任意组件选项。以组件使用混合对象时，所有混合对象的选项将被混入该组件本身的选项。
    props: {
      bgImage: {  //背景图
        type: String,
        default: ''
    ...
    methods: {
      ...
      selectItem(item, index) { // 接收从子组件song-list派发出来的事件
        this.selectPlay({
  ...
  .music-list
    ...
    .bg-image   //头部背景图样式的设置
      ...
      .filter
        ...
        width: 100%
        height: 100%
        background: rgba(208, 123, 234, 0.4)
    .bg-layer
      position: relative
      height: 100%
      background: $color-background

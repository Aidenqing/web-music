<template>
  <scroll ref="suggest" class="suggest" :data="result" :pullup="pullup" :beforeScroll="beforeScroll"
          @scrollToEnd="searchMore" @beforeScroll="listScroll"> <!-- @scrollToEnd="searchMore" 若监听到滚动到底部了，则执行加载更多-->
    ...
    <div v-show="!hasMore && !result.length" class="no-result-wrapper"> <!--边界处理无数据情况-->
      <no-result title="抱歉，暂无搜索结果"></no-result> <!--若title里传的是一个固定值的话，是不需要用 :title 的-->
    </div>
  </scroll>
</template>
    ...
    methods: {
      ...
      _genResult(data) {  // 处理结果数据
        ...
        if (data.zhida && data.zhida.singerid) { // 歌手数据
          ret.push({...data.zhida, ...{type: TYPE_SINGER}})  // es6 扩展运算符，将某些数据结构转为数组

...
export const selectPlay = function ({commit, state}, {list, index}) {  //选择播放     有两个参数，其中 {commit, state}参数是一个可以解构成commit方法, state属性的对象
...
export const insertSong = function ({commit, state}, song) {  // 插入歌曲(点击搜索结果列表中的歌曲时用到，这里需要修改playlist，sequenceList，currentIndex)
...
export const modifyCurrentSong = function ({commit,state},url) {
    //let playlist = Object.assign(JSON.parse(JSON.stringify(state.playlist)))
    let playlist =state.playlist.slice()
    let currentIndex = state.currentIndex
